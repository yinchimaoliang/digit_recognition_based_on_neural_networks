...
root = "data/"
train_list = root + "mnist/train/train.txt"
...
def Lenet(img_list, batch_size, include_acc=False):
    ...
    # 卷积层
    conv2 = L.Convolution(relu1, kernel_size=5, stride=1, num_output=50, pad=0, weight_filler=dict(type='xavier'))
    # 激活函数层
    relu2 = L.ReLU(conv2, in_place=True)
    # 池化层
    pool1 = L.Pooling(relu2, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    # 第二层：卷积层
    conv3 = L.Convolution(pool1, kernel_size=5, stride=1, num_output=20, pad=0, weight_filler=dict(type='xavier'))
    # 激活函数层
    relu3 = L.ReLU(conv3, in_place=True)
    # 卷积层
    conv4 = L.Convolution(relu3, kernel_size=5, stride=1, num_output=50, pad=0, weight_filler=dict(type='xavier'))
    # 激活函数层
...
def gen_solver(solver_file,train_net,test_net):
    ...
    s.max_iter = 18760  #10 epochs , 938*10，最大训练次数
